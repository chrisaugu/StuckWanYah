...
app.route('/foo')
		.get(isLoggedIn, function (req, res) {
				if (!req.session.user_id) {
						res.render('connect.html', {connected: false});
...
var rateImages = async function(config){
		...
		var voter = await getIdFromImageId(config.params.session.user_id);
...
var tieBreaker = function(config){
		...
		var voter = getIdFromImageId(config.params.session.user_id);
...
function getIdFromImageId(userid) {
...
function /* step: 2 */ getUserDetailsFromFacebook(userId){
		return request({
				...
				if (error) throw new Error(error); /*getUserDetailsFromFacebook(userId)*/;
				if (response) {
						var bodyObj = JSON.parse(body);
...
// run an aggregate query that will get all of the people who I vote for.
...
// run an aggregate query that will get all of the people who voted me.
...
app.get("/api/v1/photos/twophotos", function(req, res) {
	var photos = [{
		"imageId": 1,
		"picture":"friend1",
		"ratings":2,
