...
Sweetlips.photos.methods(['get', 'put', 'post', 'delete']).register(app, '/api/v1/photos/all');
// Register hits model
Sweetlips.hits.methods(['get', 'put', 'post', 'delete']).register(app, '/api/v1/hits');
...
app.route("/api/v1/auth/facebook/webhook").get(function (req, res) {
...
app.route("/api/v1/auth/facebook/webhook").post(function (req, res) {
 ...
 */
app.route("/api/v1/photos").get(function (req, res, next) {
    ...
    Sweetlips.photos.find({
        ...
        Sweetlips.photos.find({
            ...
            if (photos.length === 2) return res.send(photos);
 ...
 */
app.route('/api/v1/photos').put(function (req, res, next) {
 ...
 */
app.route('/api/v1/photos/top').get(function (req, res, next) {
 ...
 */
app.route('/api/v1/stats').get(function (req, res, next) {
    async.parallel([function (callback) {
        ...
        // total votes cast
        Sweetlips.photos.aggregate({ $group: { _id: null, total: { $sum: '$wins' } } }, function (err, winsCount) {
            callback(err, winsCount[0].total);
        });
    }, function (callback) {
        // total page hits
        Sweetlips.hits.aggregate({ $group: { _id: null, total: { $sum: '$hits' } } }, function (err, pageHits) {
 ...
 */
app.route("/api/v1/submit").post(function (req, res, next) {
 ...
 * Update site hits
 */
app.route("/api/v1/hits").put(function (req, res, next) {
    processPageHits({
        params: req,
        success: function (obj) {
            res.send(obj);
...
app.route("/api/v1/auth/facebook/connect").get(function (req, res, next) {
...
app.route("/api/v1/photos/instance").post(function (req, res, next) {
...
var getTwoRandomPhotos = function (config) {
    ...
    var fields = {};
    var options = { limit: 2 };
    Sweetlips.photos.findRandom(filter, fields, options).then(function (photos) {
        ...
        /*Sweetlips.photos
            ...
            .limit(2)
            .exec()*/
        Sweetlips.photos.findRandom({ gender: gender, voted: false, is_blocked: false }, {}, { limit: 2 }).then(function (photos) {
            if (photos.length === 2) {
                randomImages = photos;
            }
...
var top10HottestFriends = function (config) {
    ...
    Sweetlips.photos.find().sort('-ratings').where({ "gender": gender }).limit(10)
    //.select("gender")
    .exec(function (err, ranks) {
        if (err) console.error(err);

        var rankCount = _.countBy(ranks, function (rank) {
            return rank;
        });
        var max = _.max(rankCount, function (rank) {
            return rank;
        });
...
var topTenWinings = function (config) {
    ...
    Sweetlips.photos.find(conditions).where({ "gender": gender }).sort('-wins') // Sort in descending order (highest wins on top)
    .limit(10).exec().then(function (photos) {
...
var processPageHits = function (config) {
    Sweetlips.hits.update({ page: config.params.body.page }, { $inc: { hits: 1 } }, { upsert: true }).then(function (hit) {
...
function processBlock(userId) {
    ...
    Sweetlips.photos.findOne({ image_id: userId }, function (err, user) {
        if (err) {
            attempts++;
            if (attempts > 2) sendMessage(userId, { text: "Sorry it's my fault. Try again later." });
            attempts = 0;
...
function blockPhotoById(id) {
    Sweetlips.photos.update({ image_id: id }, { $set: { '$is_blocked': true } }, function (err) {
        if (err) throw err;
        return true;
    });
}
function unblockPhotoById(id) {
    Sweetlips.photos.update({ image_id: id }, { $set: { '$is_blocked': false } }, function (err) {
        if (err) throw err;else if (!err) return 1;
    });
}
/**
...
function shimOrhim(gender) {
    ...
    if (gender === "male") return gender = "female";else if (gender === "female") return gender = "male";
}
        ...
        return new Promise(function (resolve, reject) {
            FB.login(function (result) {
                if (result.authResponse) {
                    return $.post('http://localhost:5000/api/v1/auth/facebook', { access_token: result.authResponse.accessToken }).toPromise().then(function (response) {
