...
var random = require('./sweetlips-random');
...
SweetLipsSchema.statics.random = function(filter, fields, options, callback) {
    let _this = this;

    console.log(filter)

    _this.estimatedDocumentCount(filter, function(err, count) {
        if (err) {
            return callback(err);
        }
